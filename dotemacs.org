...
** mhtml-mode
   ...
   this relies on the buffer-local variable 'sgml-quick-keys' being set to 'close
   so use setq-default so new buffers will set the buffer-local variable correctly
   #+NAME: mhtml-mode-config
   #+BEGIN_SRC emacs-lisp
     (setq-default sgml-quick-keys 'close)
   #+END_SRC
* Packages
